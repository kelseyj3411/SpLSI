...
def plot_one_simul(
    ...
    start_time = time.time()
    model_v = splsi.SpLSI(
        lamb_start=0.0001, step_size=1.35, grid_len=30, method="nonspatial", verbose=0
    )
    model_v.fit(X, K, edge_df, weights)
    time_v = time.time() - start_time
    ...
    start_time = time.time()
    model_splsi = splsi.SpLSI(
        lamb_start=0.0001, step_size=1.35, grid_len=30, step="two-step", verbose=0
    )
    model_splsi.fit(X, K, edge_df, weights)
    time_splsi = time.time() - start_time
    print(f"CV Lambda is {model_splsi.lambd}")
    ...
    if plot_what:
        ...
        for j, ax in enumerate(axes):
            ...
            coords_df[f"w{j+1}"] = w
            sns.scatterplot(
                x="x", y="y", hue=f"w{j+1}", data=coords_df, palette="viridis", ax=ax
            )
...
def run_simul_fixed_lambda(
    nsim, lambd_grid, N=10, n=1000, p=30, K=3, r=0.05, m=5, phi=0.1
):
    results = []
