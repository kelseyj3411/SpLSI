...
def _update_xis(
    ...
    if n_parallel_processes > 1:
        with Pool(n_parallel_processes) as pool:
            ...
            sample_diff_matrices = [
                difference_matrices[sample_idx] for sample_idx in unique_idxs
            ]
...
def train(
    ...
    lda.topic_weights = pd.DataFrame(
        lda.fit_transform(sample_features.values),
...
def train_no_xi(
    sample_features, n_topics, max_lda_iter=5, n_iters=3, n_parallel_processes=1
):
    """Train a Spatial-LDA model.
