...
def plot_bcell_topic_multicolor(ax, sample_idx, topic_weights, spleen_dfs, name_list, lamb):
...
def get_PAS(weights_df, edge):
  ...
  edge_sets = pd.DataFrame({'src': cell_idx[src_nodes.astype(int)], 'dst': cell_idx[dst_nodes.astype(int)]})

  joined_df = edge_sets.merge(cell_topics, left_on='src', right_on='cell')\
                       .merge(cell_topics, left_on='dst', right_on='cell', suffixes=('_src', '_dst'))
  cell_counts = joined_df.groupby('src').apply(lambda x: (x['topic_src'] != x['topic_dst']).mean())
...
def get_avg_corr(weights_df1, weights_df2, n_topics):
    ...
    for i, n_topic in enumerate(n_topics):
        order = list(get_component_mapping(stats_list_SLDA[i], stats_list_tscore[i]).values())
        tscore = weights_df2[n_topic]
        tscore_rearranged = tscore[order]
        slda = weights_df1[n_topic]
        corr = np.abs([np.corrcoef(slda[:, j], tscore_rearranged[:, j])[0, 1] for j in range(slda.shape[1])])
