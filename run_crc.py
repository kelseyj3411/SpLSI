...
def preprocess_crc_(minx, maxx, miny, maxy,
                    coord_df, edge_df, D, phi, plot_sub, s):
    ...
    if maxx*maxy < 1.0 or minx*miny > 0.0:
        ...
        n = len(nodes)
        edge_df_ = edge_df_.loc[(edge_df_['src'].isin(nodes)) & (edge_df_['tgt'].isin(nodes))]
        ...
        weights = csr_matrix((edge_df__['weight'].values, (edge_df__['src'].values, edge_df__['tgt'].values)),shape=(n,n))
        X = X.iloc[nodes]
        edge_df = edge_df__
        coord_df = samp_coord
...
def run_crc(coord_df, edge_df, D, K, phi):
    ...
    model_splsi = splsi.SpLSI(lamb_start=0.0001, step_size=1.17, grid_len=50, eps=1e-05, step="two-step", verbose=1)
