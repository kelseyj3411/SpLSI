...
def preprocess_spleen_(
        minx, maxx, miny, maxy,
        tumor, spleen_coord, spleen_edge, spleen_D, phi, plot_sub, s):
    # normalize coordinate to (0,1)
    spleen_coord.loc[tumor,['x','y']] = normaliza_coords(spleen_coord.loc[tumor])

    # get weight
    spleen_edge.loc[tumor,['weight']] = dist_to_exp_weight(spleen_edge.loc[tumor], spleen_coord.loc[tumor], phi)
    ...
    n = X.shape[0]
    weights = csr_matrix((edge_df['weight'].values, (edge_df['src'].values, edge_df['tgt'].values)),shape=(n,n))
    ...
    if maxx*maxy < 1.0 or minx*miny > 0.0:
        ...
        # subset nodes
        samp_coord = coord_df.loc[(coord_df['x']<maxx) & (coord_df['x']>minx) & (coord_df['y']<maxy) & (coord_df['y']>miny)]
        nodes = samp_coord.index.tolist()
        n = len(nodes)
        edge_df = edge_df.loc[(edge_df['src'].isin(nodes)) & (edge_df['tgt'].isin(nodes))]
        ...
        weights = csr_matrix((edge_df_['weight'].values, (edge_df_['src'].values, edge_df_['tgt'].values)),shape=(n,n))
        X = X.iloc[nodes]
        edge_df = edge_df_
        coord_df = samp_coord
...
def preprocess_spleen(tumor, coord_df, edge_df, D, phi):
    ...
    # get weight
    edge_df.loc[tumor,['weight']] = dist_to_exp_weight(edge_df.loc[tumor], coord_df.loc[tumor], phi)
...
def run_spleen(minx, maxx, miny, maxy,
               tumor, coord_df, edge_df, D, K, phi, plot_sub, s):
    results = []
    #X, edge_df, coord_df, weights, n, nodes = preprocess_spleen(tumor, coord_df, edge_df, D, phi)
    X, edge_df, coord_df, weights, n, nodes = preprocess_spleen_(minx, maxx, miny, maxy, 
                                                                 tumor, coord_df, edge_df, D, phi, plot_sub, s)
    # SPLSI
    start_time = time.time()
    model_splsi = splsi.SpLSI(lamb_start=0.0001, step_size=1.17, grid_len=50, step="two-step", verbose=1)
    model_splsi.fit(X.values, K, edge_df, weights)
    time_splsi = time.time() - start_time

    # PLSI
    start_time = time.time()
    model_plsi = splsi.SpLSI(lamb_start=0.0001, step_size=1.17, grid_len=50, method="nonspatial", verbose=1)
    ...
    print("Running SLDA...")
    start_time = time.time()
    model_slda = utils.spatial_lda.model.train(sample_features=X,
...
if __name__ == "__main__":
    ...
    for tumor in tumors:
        ...
        aligned_models = plot_topic(spatial_models, ntopics_list, fig_root, 'BALBc-1', 15)
